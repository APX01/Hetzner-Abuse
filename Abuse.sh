...
fetch_ip_ranges() {
    ...
    if ! curl -s --retry 3 --retry-delay 5 'https://raw.githubusercontent.com/APX01/Hetzner-Abuse/main/Ips.txt' -o /tmp/Ips.txt; then
...
IP_LIST=\$(curl -s --retry 3 --retry-delay 5 'https://raw.githubusercontent.com/APX01/Hetzner-Abuse/main/Ips.txt')
if [ -n "\$IP_LIST" ]; then
while IFS= read -r RANGE; do
if [[ -n "\$RANGE" && ! "\$RANGE" =~ ^[[:space:]]*# && "\$RANGE" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/[0-9]+$ ]]; then
ipset add AS_Blocker "\$RANGE"
